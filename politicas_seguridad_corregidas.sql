-- POLÍTICAS DE SEGURIDAD CORREGIDAS - TYPE CASTING
...
USING (
  ...
  EXISTS (SELECT 1 FROM usuarios WHERE id::uuid = auth.uid() AND rol IN ('superadmin', 'superadministrador'))
);
...
USING (id::uuid = auth.uid() OR rol IN ('superadmin', 'superadministrador'));

CREATE POLICY "Superadmin pueden crear usuarios"
ON usuarios FOR INSERT
WITH CHECK (EXISTS (SELECT 1 FROM usuarios WHERE id::uuid = auth.uid() AND rol IN ('superadmin', 'superadministrador')));

CREATE POLICY "Usuarios pueden actualizar sus propios datos"
ON usuarios FOR UPDATE
USING (id::uuid = auth.uid() OR EXISTS (SELECT 1 FROM usuarios WHERE id::uuid = auth.uid() AND rol IN ('superadmin', 'superadministrador')));
...
USING (EXISTS (SELECT 1 FROM usuarios WHERE id::uuid = auth.uid() AND rol IN ('superadmin', 'superadministrador')));
